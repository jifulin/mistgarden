...
- 🎤 **逐字歌词**：播放音乐时，歌词会接管首页「一言」的位置——当前行按播放进度从左往右填充（卡拉 OK 效果），下一行用暗色预告即将唱到的歌词；换行时新行从下方滑入，鼠标悬停歌词时的提示由看板娘在气泡里说出，暂停后自动还原成一言
- 🌸 **樱花和粒子特效**，切到后台时暂停
- 📱 **响应式适配**：支持手机、平板和桌面端
...
assets/
  ...
  lyrics.js                逐字歌词：播放时接管一言的位置，两行歌词 + 填充 / 滑入动画
...
| 歌词样式（当前行 / 下一行的字号、上移距离、滑入速度） | `style.css` 里 `.hk-lyric` 和 `.hk-lyric-next` |
